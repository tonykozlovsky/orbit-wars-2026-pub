...
_orbit_wars_trace_format_double = None


def _orbit_wars_reset_trace_fmt_d(v):
    fmt = _orbit_wars_trace_format_double
    if fmt is not None:
        return str(fmt(float(v)))
    return str(float(v))


def _orbit_wars_sim_subtrace_append(_line):
    return None


def _orbit_wars_trace_emit_state(stage, block, planets, fleets):
    for p in sorted(planets, key=lambda row: int(row[0])):
        _orbit_wars_sim_subtrace_append(
            str(stage)
            + "\t"
            + str(block)
            + "\tplanet\t"
            + "\t".join(
                (
                    str(int(p[0])),
                    str(int(p[1])),
                    str(_orbit_wars_reset_trace_fmt_d(float(p[2]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(p[3]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(p[4]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(p[5]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(p[6]))),
                )
            )
        )
    for f in sorted(fleets, key=lambda row: int(row[0])):
        _orbit_wars_sim_subtrace_append(
            str(stage)
            + "\t"
            + str(block)
            ...
            + "\t".join(
                (
                    str(int(f[0])),
                    str(int(f[1])),
                    str(_orbit_wars_reset_trace_fmt_d(float(f[2]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(f[3]))),
                    str(_orbit_wars_reset_trace_fmt_d(float(f[4]))),
                    str(int(f[5])),
                    str(_orbit_wars_reset_trace_fmt_d(float(f[6]))),
                )
            )
        )
...
def interpreter(state, env):
    ...
    if not hasattr(obs0, "planets") or not obs0.planets:
        ...
        _orbit_wars_trace_emit_state("30", "reset_init", obs0.planets, obs0.fleets)
        return state
    ...
    _orbit_wars_trace_emit_state("31", "step_after_pre_launch", obs0.planets, obs0.fleets)
    ...
    _orbit_wars_trace_emit_state("31", "step_after_launch", obs0.planets, obs0.fleets)

    # 1. Production
    for planet in obs0.planets:
        if planet[1] != -1:
            planet[5] += planet[6]
    _orbit_wars_trace_emit_state("31", "step_after_production", obs0.planets, obs0.fleets)
    ...
    for fleet in obs0.fleets:
        ...
        if point_to_segment_distance((CENTER, CENTER), old_pos, new_pos) < SUN_RADIUS:
            fleets_to_remove.append(fleet)
            continue
    _orbit_wars_trace_emit_state("31", "step_after_fleet_movement", obs0.planets, obs0.fleets)
    ...
    _orbit_wars_trace_emit_state("31", "step_after_planet_motion_apply", obs0.planets, obs0.fleets)
    ...
    if expired_comet_pids:
        ...
        obs0.comets = [g for g in obs0.comets if g["planet_ids"]]
    _orbit_wars_trace_emit_state("31", "step_after_expired_comet_removal", obs0.planets, obs0.fleets)

    obs0.fleets = [f for f in obs0.fleets if f not in fleets_to_remove]
    _orbit_wars_trace_emit_state("31", "step_after_fleet_prune", obs0.planets, obs0.fleets)
    _orbit_wars_trace_emit_state("31", "step_pre_combat", obs0.planets, obs0.fleets)
    ...
    _orbit_wars_trace_emit_state("31", "step_after_combat", obs0.planets, obs0.fleets)
    ...
    if terminated:
        ...
        if len(alive_players) <= 1:
            winner = next(iter(alive_players)) if alive_players else None
            for i in range(num_agents):
                state[i].reward = 1 if winner is not None and i == winner else -1
        else:
            scores = [0] * num_agents
            for p in obs0.planets:
                if p[1] != -1:
